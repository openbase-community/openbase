...
def _ensure_reasoning_configurable() -> None:
    try:
        backend = configured_execution_backend()
    except Exception:
        return
    if backend == CLAUDE_CODE_BACKEND:
        raise click.ClickException(dispatcher_config.CLAUDE_BACKEND_REASONING_ERROR)
...
def dispatcher_reasoning(level: str | None) -> None:
    ...
    _ensure_reasoning_configurable()
