...
Code-sync conflict helpers also default to a live host-state path. Redirect
that default per test so a unit test which intentionally creates divergent
temporary repositories can never surface a product warning on the developer's
machine.
"""
