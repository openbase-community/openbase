...
| `~/.codex/config.toml` | User, `openbase-coder setup` | Normal Codex config; setup registers a `[mcp_servers.super-agents]` table in it. The optional `--install-normal-agent-hooks` also adds the trusted session provenance hook. With `--link-codex-config`, this is the target of the Openbase service Codex config symlink and receives Openbase permission overrides |
