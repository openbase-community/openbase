...
| `--install-normal-agent-hooks/--no-install-normal-agent-hooks` | prompted with default `false` in interactive setup; otherwise `false` | Also register the `Agent-Thread-Id` SessionStart hook in normal `~/.codex/config.toml` and `~/.claude/settings.json`. Openbase-managed homes are always configured |
...
14. Installs the bundled `inject-session-id.sh` SessionStart hook script into `~/.openbase/hooks/` and registers it in both Openbase agent homes: a `hooks.SessionStart` entry in `~/.openbase/claude_config/settings.json` and a trusted `[[hooks.SessionStart]]` hook (with its `[hooks.state]` trust entry) in `~/.openbase/codex_home/config.toml`. Interactive setup asks whether to register the same hook in normal `~/.claude/settings.json` and `~/.codex/config.toml`, defaulting to no; non-interactive setup only does so with `--install-normal-agent-hooks`. The hook feeds each session's thread/session ID back into the conversation together with the instructions for using it, so agents stamp commits with the `Agent-Thread-Id` trailer without needing any standing `AGENTS.md` rule.
...
18. Registers the Super Agents MCP server in the user's **normal** agent homes — a `[mcp_servers.super-agents]` table in `~/.codex/config.toml` and an `mcpServers.super-agents` entry in `~/.claude.json` — regardless of `--link-codex-config`. Each entry identifies its spawning backend (`codex` or `claude_code`) so child threads inherit it unless a launch explicitly overrides the backend. Only the MCP entry is written; normal permissions and settings are never touched. You may remove the entry; an explicit setup re-run restores it.
...
24. Leaves Openbase Cloud registration to the later login/pairing flow. Use
...
- Session provenance hooks are always installed in the Openbase-managed agent homes. Normal `~/.codex/config.toml` and `~/.claude/settings.json` receive the hook only after the interactive opt-in or with `--install-normal-agent-hooks` (unless an explicit config-link option makes the Openbase and normal files the same file).
