...
15. Records profile paths, `CODEX_APP_SERVER_URL=unix://` on macOS/Linux, and `LIVEKIT_CODEX_THREAD_CWD` in the Openbase `.env`. Existing default `ws://127.0.0.1:4500` installations migrate to the standard socket; custom WebSocket endpoints are preserved. Backend model/provider configuration is applied per thread through profiles. The shared daemon receives no global Openbase configuration overrides. The visible `openbase_cloud` backend uses Claude Code; the internal Codex proxy profile supports `openbase_cloud_codex`.
...
- Your own `~/.codex/AGENTS.md` and `~/.claude/CLAUDE.md` are read by the respective backends and left unchanged by setup.
