...
11. Installs the bundled `inject-session-id.sh` SessionStart hook script into `~/.openbase/hooks/` and registers it in both shared agent homes: a trusted `[[hooks.SessionStart]]` hook (with its `[hooks.state]` trust entry) in `~/.codex/config.toml` and a `hooks` entry in `~/.claude/settings.json`. The hook feeds each session's thread/session ID back into the conversation together with the instructions for using it, so agents stamp commits with the `Agent-Thread-Id` trailer without needing any standing `AGENTS.md` rule.
